...
export default function SearchSales({changeQuery, styles, show, value}) {
    const [query, setQuery] = useState("");
    const {width} = useWindowDimensions();
    const onChangeText=(text)=>{
        setQuery(text);
        text ? show(true) : show(false);
        value(text);
    }
    return (
        <View style={[styles2.mainContainer, {...styles}]}>
            <View style={[styles2.subContainer]}>
                <TextInput onChangeText={onChangeText} placeholder='Buscar ...' value={query} style={styles2.inputText}/>
