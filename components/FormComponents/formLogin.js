        ...
        <View style={styles.inputContainer}>
            {
                inputs.map((val,key)=>{
                    if (val.inputText === "password") {
                        return (
                            <View key={key}>
                                <Text style={styles.labelTextStyle}>{val.titleInput ? val.titleInput :  "Contraseña"}</Text>
                    ...
                    if (val.inputText === "document") {
                        return (
                            <View key={key}>
                                <Text style={styles.labelTextStyle}>{val.titleInput ? val.titleInput : "Documento"}</Text>
                    ...
                    if (val.inputText==="phone") {
                        return (
                            <View key={key}>
                                <Text style={styles.labelTextStyle}>{val.titleInput ? val.titleInput : "Telefono"}</Text>
                    ...
                    if (val.inputText==="email") {
                        return(
                            <View key={key}>
                                <Text style={styles.labelTextStyle}>{val.titleInput ? val.titleInput : "Correo electronico"}</Text>
                    ...
                    if (val.inputText === "username") {
                        ...
                        <View  key={key} style={{flexDirection:'column'}}>
                            <Text style={styles.labelTextStyle}>{val.titleInput ? val.titleInput : "Usuario"}</Text>
                            <InputTextUsername changeDataAdmin={text => changeDataAdmin(text, val.inputText)} inputText={val.inpuText} {...val} />
                        </View>)
                    }
                    return (
                        <View key={key} >
                            <Text style={styles.labelTextStyle}>{val.titleInput ? val.titleInput : ""}</Text>
